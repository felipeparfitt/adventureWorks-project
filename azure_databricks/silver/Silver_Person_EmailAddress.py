...
# Creating the Person_EmailAddress table in silver layer
spark.sql(f"""
...
def tranforming_EmailAddress(
    df_EmailAddress: DataFrame,
    sink_table_name: str,
    primary_keys: list,
    expected_schema: StructType
    ) -> DataFrame:
    """
    Transforms and cleans data from the Person.EmailAddress table.

    Parameters:
        df_EmailAddress (DataFrame): The DataFrame containing the EmailAddress data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.person_EmailAddress: ", end='')

    # Removing duplicates through primary keys
    df_EmailAddress_dedup = df_deduplicate(
        df=df_EmailAddress, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_EmailAddress_fillna = df_EmailAddress_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_EmailAddress_fillna = df_EmailAddress_fillna.withColumn(
        '_process_timestamp',
    ...
    df_EmailAddress_cr = df_EmailAddress_fillna.filter(F.expr(constrains_conditions))
    df_EmailAddress_qr = df_EmailAddress_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_EmailAddress_casted = df_EmailAddress_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_EmailAddress_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_EmailAddress_casted
...
df_EmailAddress_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_EmailAddress_transformed = tranforming_EmailAddress(
    df_EmailAddress=df_EmailAddress_bronze,
...
upsert_delta_table(
  df_source_table=df_EmailAddress_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
