...
spark.sql(f"""
    CREATE EXTERNAL TABLE IF NOT EXISTS {silver_target_table_name} (
        CurrencyCode STRING NOT NULL PRIMARY KEY,
...
def tranforming_Currency(
    ...
    """
    Transforms and cleans data from the Sales.Currency table.

    Parameters:
        df_Currency (DataFrame): The DataFrame containing the Currency data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.sales_Currency: ", end='')

    # Removing duplicates through primary keys
    df_Currency_dedup = df_deduplicate(
        df=df_Currency, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_Currency_fillna = df_Currency_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_Currency_fillna = df_Currency_fillna.withColumn(
        '_process_timestamp',
    ...
    df_Currency_cr = df_Currency_fillna.filter(F.expr(constrains_conditions))
    df_Currency_qr = df_Currency_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_Currency_casted = df_Currency_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_Currency_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_Currency_casted
...
df_Currency_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_Currency_transformed = tranforming_Currency(
    df_Currency=df_Currency_bronze,
...
upsert_delta_table(
  df_source_table=df_Currency_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
