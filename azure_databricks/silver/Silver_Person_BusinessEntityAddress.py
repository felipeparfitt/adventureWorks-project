...
expected_schema = StructType([
    ...
    StructField("AddressTypeID", IntegerType(), False),
...
# Creating the Person_BusinessEntityAddress table in silver layer
spark.sql(f"""
...
def tranforming_BusinessEntityAddress(
    df_BusinessEntityAddress: DataFrame,
    sink_table_name: str,
    primary_keys: list,
    expected_schema: StructType
    ) -> DataFrame:
    """
    Transforms and cleans data from the Person.BusinessEntityAddress table.

    Parameters:
        df_BusinessEntityAddress (DataFrame): The DataFrame containing the BusinessEntityAddress data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.person_BusinessEntityAddress: ", end='')

    # Removing duplicates through primary keys
    df_BusinessEntityAddress_dedup = df_deduplicate(
        df=df_BusinessEntityAddress, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_BusinessEntityAddress_fillna = df_BusinessEntityAddress_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_BusinessEntityAddress_fillna = df_BusinessEntityAddress_fillna.withColumn(
        '_process_timestamp',
    ...
    df_BusinessEntityAddress_cr = df_BusinessEntityAddress_fillna.filter(F.expr(constrains_conditions))
    df_BusinessEntityAddress_qr = df_BusinessEntityAddress_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_BusinessEntityAddress_casted = df_BusinessEntityAddress_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_BusinessEntityAddress_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_BusinessEntityAddress_casted
...
df_BusinessEntityAddress_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_BusinessEntityAddress_transformed = tranforming_BusinessEntityAddress(
    df_BusinessEntityAddress=df_BusinessEntityAddress_bronze,
...
upsert_delta_table(
  df_source_table=df_BusinessEntityAddress_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
