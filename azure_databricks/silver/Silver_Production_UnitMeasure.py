...
expected_schema = StructType([
    StructField("UnitMeasureCode", StringType(), False),
...
# Creating the Production_UnitMeasure table in silver layer
spark.sql(f"""
    CREATE EXTERNAL TABLE IF NOT EXISTS {silver_target_table_name} (
        UnitMeasureCode STRING NOT NULL PRIMARY KEY,
...
def tranforming_UnitMeasure(
    df_UnitMeasure: DataFrame,
    sink_table_name: str,
    primary_keys: list,
    expected_schema: StructType
    ) -> DataFrame:
    """
    Transforms and cleans data from the Production.UnitMeasure table.

    Parameters:
        df_UnitMeasure (DataFrame): The DataFrame containing the UnitMeasure data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.production_UnitMeasure: ", end='')

    # Removing duplicates through primary keys
    df_UnitMeasure_dedup = df_deduplicate(
        df=df_UnitMeasure, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_UnitMeasure_fillna = df_UnitMeasure_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_UnitMeasure_fillna = df_UnitMeasure_fillna.withColumn(
        '_process_timestamp',
    ...
    df_UnitMeasure_cr = df_UnitMeasure_fillna.filter(F.expr(constrains_conditions))
    df_UnitMeasure_qr = df_UnitMeasure_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_UnitMeasure_casted = df_UnitMeasure_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_UnitMeasure_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_UnitMeasure_casted
...
df_UnitMeasure_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_UnitMeasure_transformed = tranforming_UnitMeasure(
    df_UnitMeasure=df_UnitMeasure_bronze,
...
upsert_delta_table(
  df_source_table=df_UnitMeasure_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
