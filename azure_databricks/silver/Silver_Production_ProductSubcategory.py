...
# Creating the Production_ProductSubcategory table in silver layer
spark.sql(f"""
...
def tranforming_ProductSubcategory(
    df_ProductSubcategory: DataFrame,
    sink_table_name: str,
    primary_keys: list,
    expected_schema: StructType
    ) -> DataFrame:
    """
    Transforms and cleans data from the Production.ProductSubcategory table.

    Parameters:
        df_ProductSubcategory (DataFrame): The DataFrame containing the ProductSubcategory data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.production_ProductSubcategory: ", end='')

    # Removing duplicates through primary keys
    df_ProductSubcategory_dedup = df_deduplicate(
        df=df_ProductSubcategory, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_ProductSubcategory_fillna = df_ProductSubcategory_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_ProductSubcategory_fillna = df_ProductSubcategory_fillna.withColumn(
        '_process_timestamp',
    ...
    df_ProductSubcategory_cr = df_ProductSubcategory_fillna.filter(F.expr(constrains_conditions))
    df_ProductSubcategory_qr = df_ProductSubcategory_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_ProductSubcategory_casted = df_ProductSubcategory_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_ProductSubcategory_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_ProductSubcategory_casted
...
df_ProductSubcategory_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_ProductSubcategory_transformed = tranforming_ProductSubcategory(
    df_ProductSubcategory=df_ProductSubcategory_bronze,
...
upsert_delta_table(
  df_source_table=df_ProductSubcategory_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
