...
# Creating the Production_ProductDescription table in silver layer
spark.sql(f"""
...
def tranforming_ProductDescription(
    df_ProductDescription: DataFrame,
    sink_table_name: str,
    primary_keys: list,
    expected_schema: StructType
    ) -> DataFrame:
    """
    Transforms and cleans data from the Production.ProductDescription table.

    Parameters:
        df_ProductDescription (DataFrame): The DataFrame containing the ProductDescription data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.production_ProductDescription: ", end='')

    # Removing duplicates through primary keys
    df_ProductDescription_dedup = df_deduplicate(
        df=df_ProductDescription, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_ProductDescription_fillna = df_ProductDescription_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_ProductDescription_fillna = df_ProductDescription_fillna.withColumn(
        '_process_timestamp',
    ...
    df_ProductDescription_cr = df_ProductDescription_fillna.filter(F.expr(constrains_conditions))
    df_ProductDescription_qr = df_ProductDescription_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_ProductDescription_casted = df_ProductDescription_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_ProductDescription_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_ProductDescription_casted
...
df_ProductDescription_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_ProductDescription_transformed = tranforming_ProductDescription(
    df_ProductDescription=df_ProductDescription_bronze,
...
upsert_delta_table(
  df_source_table=df_ProductDescription_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
