...
# Creating the Production_ProductListPriceHistory table in silver layer
spark.sql(f"""
...
""")
constraints = [
        "ADD CONSTRAINT CK_ProductCostHistory_StandardCost CHECK (ListPrice >= 0.00)",
...
def tranforming_ProductListPriceHistory(
    df_ProductListPriceHistory: DataFrame,
    sink_table_name: str,
    primary_keys: list,
    expected_schema: StructType
    ) -> DataFrame:
    """
    Transforms and cleans data from the Production.ProductListPriceHistory table.

    Parameters:
        df_ProductListPriceHistory (DataFrame): The DataFrame containing the ProductListPriceHistory data to be transformed.
    ...
    """
    print(f"Transforming the adventure_dev.bronze.production_ProductListPriceHistory: ", end='')

    # Removing duplicates through primary keys
    df_ProductListPriceHistory_dedup = df_deduplicate(
        df=df_ProductListPriceHistory, 
        primary_keys=primary_keys,
        order_col='ModifiedDate'
    )
    # Setting default values to null values
    df_ProductListPriceHistory_fillna = df_ProductListPriceHistory_dedup.withColumn(
        'ModifiedDate',
        F.when(F.col('ModifiedDate').isNull(), F.current_timestamp())
        .otherwise(F.col('ModifiedDate'))
    )
    df_ProductListPriceHistory_fillna = df_ProductListPriceHistory_fillna.withColumn(
        '_process_timestamp',
    ...
    df_ProductListPriceHistory_cr = df_ProductListPriceHistory_fillna.filter(F.expr(constrains_conditions))
    df_ProductListPriceHistory_qr = df_ProductListPriceHistory_fillna.filter(~F.expr(constrains_conditions))
    ...
    df_ProductListPriceHistory_casted = df_ProductListPriceHistory_cr.select(*select_exprs)
    
    # Verify expected schema
    verify_schema(df_ProductListPriceHistory_casted, expected_schema)

    print("Success !!")
    print("*******************************")

    return df_ProductListPriceHistory_casted
...
df_ProductListPriceHistory_bronze = spark.read.format("delta").table(bronze_source_table_name)
print("Success!!")
print("*******************************")

# Tranforming bronze layer
df_ProductListPriceHistory_transformed = tranforming_ProductListPriceHistory(
    df_ProductListPriceHistory=df_ProductListPriceHistory_bronze,
...
upsert_delta_table(
  df_source_table=df_ProductListPriceHistory_transformed,
  sink_table_name=silver_target_table_name,
  primary_keys=primary_keys
)
